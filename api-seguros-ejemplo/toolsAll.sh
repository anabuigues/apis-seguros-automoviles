...
echo "Validating with swagger-cli (fast)"
docker run --rm \
--name apitools \
-v ${PWD}:/apis \
...
echo "Validating with openapi-generator (slow)"
docker run --rm \
--name apitools \
-v ${PWD}:/apis \
...
openapi-generator generate -i path-to-single-file-with-spec \
    -g aspnetcore \
    --git-user-id GitUser \
    --git-repo-id GitRepo \
    --config /apis/openapi-generator-aspnetcore-config.json \
    -o output-directory